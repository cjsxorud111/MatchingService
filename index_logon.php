...
<html>
   <a href=""></a>
    <head>
        ...
        <script>
            $(document).ready(function(){ //페이지가 시작할때바로 id=toto인 태그에 main_article.php를 실행시켜줍니다. 
                $("#toto").load("main_article.php"); 
            });
            function career_ranking(){ //이 밑은버튼을 클릭했을때 onclick함수로 페이지를 실행할 함수들 
                $("#toto").load("career_ranking.php"); 
                return false;
            }
            function star_ranking(){
                $("#toto").load("star_ranking.php"); 
                return false;
            }
            function match_ranking(){
                $("#toto").load("match_ranking.php"); 
                return false;
            }
    ...
    <body id="target">
        ...
        <script>
            /*
            var makeAjaxCall = function (url, success_callback) { //ajax를 공통함수화
              $.ajax ({  
                type: 'POST',  
                url: url,
                data: {
            ...
            $('#expertrank').on('click',function() {
                 var lists1 = [];
                  $("input[name='check1']:checked").each(function(i){   //jQuery로 for문 돌면서 check 된값 배열에 담는다
                   lists1.push($(this).val());
                    });

                    var lists2 = [];
                  $("input[name='check2']:checked").each(function(i){   //jQuery로 for문 돌면서 check 된값 배열에 담는다
                   lists2.push($(this).val());
                });
