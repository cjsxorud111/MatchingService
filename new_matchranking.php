...
<html>
    ...
    <body id="target">
                    <article>
                       <div id="soku">
                           <h1>매치가 많이 성사된 순 정렬</h1>
                            <?php
                                echo '<br>';
                                $special1 = $_SESSION['che1']; //세션의 유저의interest값을 스페셜1변수에 담아서 검색
                                $special2 = $_SESSION['che2'];
                                ...
                                while($row = mysqli_fetch_assoc($result)){ //유저 정보 출력
                                    ...
                                    $array[$test] = $row['matchnum']; //경력을 array배열에 넣어줌
                                    $test = $test+1;
